...
pub fn create_window(message: String) {
    ...
    let window = unsafe {
        ...
        (xlib.XCreateWindow)(display, root, 0, 0, window_width, window_height, 0, 0,
    ...
    unsafe { (xlib.XSelectInput)(display, window, xlib::ExposureMask | xlib::StructureNotifyMask) };
    ...
    let mut message_lines = split_message(&xlib, font_set, &message, window_width as i32);
    ...
    loop {
        ...
        match event.get_type() {
            ...
            xlib::Expose => {
                // draw the window
                message_lines = split_message(&xlib, font_set, &message, window_width as i32);
                for (i, line) in message_lines.iter().enumerate() {
                    unsafe { (xlib.Xutf8DrawString)(display, window, font_set, gc, TEXT_MARGIN, (i as i32 + 1) * max_line_height, line.as_ptr() as *const c_char, line.to_bytes().len() as i32) };
                }
            }
            _ => ()
        }
...
fn line_width_height(xlib: &Xlib, font_set: *mut xlib::_XOC, text: &CString) -> (u16, u16) {
...
fn split_message(xlib: &Xlib, font_set: *mut xlib::_XOC, message: &String, window_width: i32) -> Vec<CString> {
        ...
        if line.is_empty() {
            ...
            for word in line.split_whitespace() {
                ...
                if let Ok(line_cstring) = CString::new(new_line.clone()) {
                    let (line_width, _) = line_width_height(&xlib, font_set, &line_cstring);
